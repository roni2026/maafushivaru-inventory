...
import {
  ...
  Calendar, RefreshCw, Filter, AlertTriangle, CheckCircle2,
  PackageX, Link, Inbox,
} from 'lucide-react'
...
export default function Orders() {
  ...
  const [avgConsMap, setAvgConsMap]       = useState({})
  ...
  // ── Average Consumption ───────────────────────────────────────────────────
  const loadAvgConsumption = useCallback(async (store, items) => {
    if (!items.length) return
    const partNums = items.map(i => codeOf(i.part_number)).filter(Boolean)
    if (!partNums.length) return
    ...
    let orderQtyMap = new Map()
    if (orderIds.length) {
      const { data: oItems } = await supabase
        .from('order_history_items')
        .select('order_id,part_number,ordered_qty')
        .in('order_id', orderIds)
      ;(oItems||[]).forEach(oi => {
        orderQtyMap.set(`${oi.order_id}::${codeOf(oi.part_number)}`, Number(oi.ordered_qty)||0)
      })
    }
    const byItem = new Map()
    obs.forEach(o => {
      const k = codeOf(o.part_number)
      if (!byItem.has(k)) byItem.set(k, [])
      byItem.get(k).push(o)
    })
    const newAvgMap = {}
    byItem.forEach((periods, partCode) => {
      const sorted = [...periods].sort((a,b) => a.week_date.localeCompare(b.week_date))
      const consumptions = []
      for (let i = 1; i < sorted.length; i++) {
        const prev = sorted[i-1], curr = sorted[i]
        const prevOrderQty = orderQtyMap.get(`${prev.order_id}::${partCode}`) || 0
        const consumption  = Number(prev.physical_qty) + prevOrderQty - Number(curr.physical_qty)
      ...
      if (consumptions.length > 0) {
        ...
        if (item) newAvgMap[item.id] = Math.round(avg * 10) / 10
        newAvgMap[`pn::${partCode}`] = Math.round(avg * 10) / 10
      }
    })
    setAvgConsMap(newAvgMap)
  }, [])
  ...
  function buildStoreRows(store, items, avgMap) {
    ...
    return items
      .filter(i => { const sn=(i.stores?.name||'').toLowerCase(); return sn===store.toLowerCase()||sn.includes(kw) })
      .sort((a,b)=>(a.name||'').localeCompare(b.name||''))
      .map((it,i)=>makeRow(it,i+1,avgMap))
  }

  const loadStoreItems = useCallback(async (store, cachedItems, cachedAvg) => {
    setLoading(true); setRows([]); setSearch('')
    try {
      const items = cachedItems || await loadAllItems()
      const avg   = cachedAvg || avgConsMap
      setRows(buildStoreRows(store, items, avg))
      // Load avg in background
      await loadAvgConsumption(store, items)
    } catch (err) { toast.error('Failed to load items: '+err.message) }
    setLoading(false)
  }, [loadAllItems, loadAvgConsumption, avgConsMap]) // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => { loadStoreItems(selectedStore) }, []) // eslint-disable-line react-hooks/exhaustive-deps

  // Update avg consumption in rows after async load
  useEffect(() => {
    if (rows.length && Object.keys(avgConsMap).length) {
      setRows(p => p.map(r => ({
        ...r,
        avgConsumption: avgConsMap[r.id] ?? avgConsMap[`pn::${codeOf(r.part_number)}`] ?? r.avgConsumption,
      })))
    }
  }, [avgConsMap]) // eslint-disable-line react-hooks/exhaustive-deps

  const selectStore = async (store) => {
    setSelectedStore(store)
    setAvgConsMap({})
    const items = allItems.length ? allItems : await loadAllItems()
    await loadStoreItems(store, items, {})
  }

  // ── Row mutation helpers ──────────────────────────────────────────────────
  const setQty       = (id, val) => {
    const n = parseFloat(val)
    if (!isNaN(n) && n >= 0) setRows(p => p.map(r => r.id===id ? {...r, ordered:n} : r))
    else if (val === '') setRows(p => p.map(r => r.id===id ? {...r, ordered:0} : r))
  }
  const adjustQty    = (id, d) => setRows(p => p.map(r => r.id===id ? {...r, ordered: Math.max(0,(r.ordered||0)+d)} : r))
  const setPhysStock = (id, val) => {
    if (val === '') { setRows(p => p.map(r => r.id===id ? {...r, physStock:''} : r)); return }
    const n = parseFloat(val)
    if (!isNaN(n) && n >= 0) setRows(p => p.map(r => r.id===id ? {...r, physStock:n} : r))
  }
  ...
  const saveOrder = async () => {
    ...
    try {
      ...
      const { error: iErr } = await supabase.from('order_history_items').insert(
        orderRows.map(r => ({
          order_id: order.id, item_id: r.id, part_number: r.part_number,
          item_name: r.name, store_name: r.store||selectedStore, unit: r.unit,
          ordered_qty: r.ordered, received_qty: 0,
          approved_qty: r.approvedQty || 0, item_status: 'submitted',
        }))
      )
      if (iErr) throw iErr

      // Save physical stock observations (separate from inventory)
      const physRows = orderRows.filter(r => r.physStock !== '' && r.physStock !== undefined && r.physStock !== null)
      if (physRows.length) {
        await supabase.from('order_physical_stock').insert(
          physRows.map(r => ({
            item_id: r.id, part_number: r.part_number, item_name: r.name,
            store_name: selectedStore, physical_qty: Number(r.physStock) || 0,
            week_date: deliveryDate, order_id: order.id, recorded_by: 'System',
          }))
        )
      }
      ...
      // Refresh avg consumption after new data
      loadAvgConsumption(selectedStore, allItems)
  ...
  const exportPDF = async () => {
    ...
    try {
      ...
      doc.text(resortName, 14, 11)
      doc.setFontSize(10); doc.setFont('helvetica','normal')
      doc.text(`${selectedStore} — ${lbl}`, 14, 20)
      autoTable(doc, {
        startY:36,
        head:[['#','Part #','Item Name','UOM','Avg/Wk','Physical Stock','Order Qty']],
        body: orderRows.map(r => [
          r.sl, codeOf(r.part_number), r.name, r.unit,
          r.avgConsumption!=null ? r.avgConsumption : '—',
          r.physStock!=='' ? r.physStock : '—',
          r.ordered,
        ]),
        headStyles:{fillColor:cyan,fontSize:8,textColor:255},
        styles:{fontSize:8},
      ...
      doc.text(`${orderRows.length} items · ${orderRows.reduce((s,r)=>s+Number(r.ordered),0)} units`, 14, y)
  ...
  const exportExcel = async () => {
    ...
    try {
      const lbl = new Date(deliveryDate).toLocaleDateString('en-US',{weekday:'long',day:'numeric',month:'long',year:'numeric'})
      await exportOrderExcel({ [selectedStore]: orderRows }, {
        resortName,
        deliveryLabel: `${selectedStore} · ${lbl}`,
  ...
  const histDateGroups = useMemo(() => {
    const g = {}
    histFiltered.forEach(o => { const d=o.delivery_date||'Unknown'; (g[d]=g[d]||[]).push(o) })
  ...
  const exportHistoryPDF = async (order) => {
    ...
    if (!oItems?.length) { toast.error('Expand the order first'); return }
    ...
    try {
      ...
      doc.text(resortName, 14, 11)
      doc.setFontSize(10); doc.setFont('helvetica','normal')
      doc.text(`${sn} — ${fmtDate(order.delivery_date)}`, 14, 20)
      autoTable(doc, {
        startY:36,
        head:[['SL','Part #','Item Name','UOM','Ordered','Approved','Received','Status']],
        body: oItems.map((oi,i) => {
          const sf = Number(oi.ordered_qty)-Number(oi.received_qty)
          return [i+1,codeOf(oi.part_number),oi.item_name,oi.unit,oi.ordered_qty,oi.approved_qty||'—',oi.received_qty,sf<=0?'Received':Number(oi.received_qty)>0?'Partial':'Pending']
        }),
        headStyles:{fillColor:cyan,fontSize:8,textColor:255},
        styles:{fontSize:8},
        alternateRowStyles:{fillColor:[248,250,252]},
      })
      doc.save(`${sn.replace(/\s+/g,'_')}_${order.delivery_date||'order'}.pdf`)
      toast.success('PDF exported')
    } catch (err) { toast.error('Export failed: '+err.message) }
  ...
  const exportHistoryExcel = async (order) => {
    ...
    try {
      const sn  = order.store_name || (order.notes||'').split(' · ')[0] || 'Order'
      const lbl = fmtDate(order.delivery_date)
      await exportOrderExcel(
        { [sn]: oItems.map((oi,i) => ({sl:i+1,part_number:oi.part_number,name:oi.item_name,unit:oi.unit,store:oi.store_name||sn,current_stock:0,pack:1,avgWeekly:0,suggested:0,ordered:oi.ordered_qty,received:oi.received_qty})) },
        { resortName, deliveryLabel:`${sn} · ${lbl}`, filename:`${sn.replace(/\s+/g,'_')}_${order.delivery_date||'order'}.xlsx` }
      )
      toast.success('Excel exported')
    } catch (err) { toast.error('Export failed: '+err.message) }
  ...
  const emailOrder = async (order) => {
    ...
    try {
      ...
      if (!key || !to) { toast.error('Set Brevo API key + recipient email in Settings'); setEmailingOrder(null); return }
      const sn  = order.store_name || (order.notes||'').split(' · ')[0] || 'Order'
      const lbl = fmtDate(order.delivery_date)
      const trs = oItems.map((oi,i) => `<tr style="background:${i%2?'#f8fafc':'#fff'}">
        <td style="padding:6px 10px;border-bottom:1px solid #e2e8f0;color:#64748b">${i+1}</td>
        <td style="padding:6px 10px;border-bottom:1px solid #e2e8f0;font-family:monospace">${codeOf(oi.part_number)}</td>
        <td style="padding:6px 10px;border-bottom:1px solid #e2e8f0">${oi.item_name}</td>
        <td style="padding:6px 10px;border-bottom:1px solid #e2e8f0;text-align:center">${oi.unit}</td>
        <td style="padding:6px 10px;border-bottom:1px solid #e2e8f0;text-align:center;font-weight:700;color:#0d9488">${oi.ordered_qty}</td>
        <td style="padding:6px 10px;border-bottom:1px solid #e2e8f0;text-align:center">${oi.approved_qty||'—'}</td>
      </tr>`).join('')
      const html = `<div style="font-family:sans-serif;max-width:720px;margin:0 auto;border:1px solid #e2e8f0;border-radius:8px;overflow:hidden">
        <div style="background:#00AEEF;padding:18px 20px"><h2 style="color:#fff;margin:0;font-size:18px">${resortName}</h2>
        <p style="color:#e0f7ff;margin:4px 0 0;font-size:13px">${sn} — ${lbl}</p></div>
        <table style="width:100%;border-collapse:collapse;font-size:13px">
          <thead><tr style="background:#f1f5f9">
            <th style="padding:8px 10px;text-align:left;font-size:11px;color:#64748b">SL</th>
            <th style="padding:8px 10px;text-align:left;font-size:11px;color:#64748b">Part #</th>
            <th style="padding:8px 10px;text-align:left;font-size:11px;color:#64748b">Item</th>
            <th style="padding:8px 10px;text-align:center;font-size:11px;color:#64748b">Unit</th>
            <th style="padding:8px 10px;text-align:center;font-size:11px;color:#64748b">Ordered</th>
            <th style="padding:8px 10px;text-align:center;font-size:11px;color:#64748b">Approved</th>
          </tr></thead>
          <tbody>${trs}</tbody>
        </table>
        <div style="padding:10px 12px;background:#f8fafc;font-size:12px;color:#64748b"><strong>${oItems.length}</strong> items</div>
      </div>`
      const r = await fetch('https://api.brevo.com/v3/smtp/email', {
        method:'POST',
        headers:{'api-key':key,'Content-Type':'application/json'},
        body:JSON.stringify({sender:{email:s.email_sender||'orders@maafushivaru.com',name:resortName},to:[{email:to}],subject:`${sn} Order — ${lbl}`,htmlContent:html})
      })
  ...
  const confirmAddToSavedOrder = async () => {
    ...
    const { error } = await supabase.from('order_history_items').insert({
      order_id: showAddToOrder, item_id: savedItem.id, part_number: savedItem.part_number,
      item_name: savedItem.name, store_name: savedItem.stores?.name||'', unit: savedItem.unit,
      ordered_qty: Number(savedQty), received_qty: 0,
    })
  ...
  const loadRcvItems = async (orderId) => {
    const isOpen = rcvExpanded === orderId
    if (isOpen) { setRcvExpanded(null); return }
    if (!rcvItems[orderId]) {
      const { data } = await supabase.from('order_history_items').select('*').eq('order_id', orderId)
      const init = {}
      ;(data||[]).forEach(oi => { init[oi.id] = { qty: oi.received_qty||0, issue: oi.issue_type||'none' } })
      setRcvItems(p => ({...p, [orderId]: data||[]}))
      setRcvInput(p => ({...p, ...init}))
    }
    setRcvExpanded(orderId)
  }

  const saveReceiving = async (order) => {
    const oItems = rcvItems[order.id]||[]
    if (!oItems.length) return
    setRcvSaving(order.id)
    try {
      const records = []
      for (const oi of oItems) {
        const inp    = rcvInput[oi.id] || { qty:0, issue:'none' }
        const rcvQty = Number(inp.qty) || 0
        const issue  = inp.issue || 'none'
        const approvedQty = Number(oi.approved_qty || oi.ordered_qty)
        if (rcvQty > approvedQty && !['wrong_item','duplicate'].includes(issue)) {
          if (!window.confirm(`${oi.item_name}: received ${rcvQty} > approved ${approvedQty}. Continue?`)) {
            setRcvSaving(null); return
          }
        }
        const recStatus = issue === 'none' ? 'received' : 'exception'
        records.push({
          order_id: order.id, order_item_id: oi.id, item_id: oi.item_id,
          part_number: oi.part_number, item_name: oi.item_name, unit: oi.unit,
          store_name: oi.store_name, ordered_qty: oi.ordered_qty, approved_qty: oi.approved_qty||0,
          received_qty: rcvQty, issue_type: issue, status: recStatus, received_by: 'System',
        })
        await supabase.from('order_history_items')
          .update({ received_qty: rcvQty, issue_type: issue, item_status: recStatus })
          .eq('id', oi.id)
      }
  ...
  const loadRcvHistory = useCallback(async () => {
    ...
    if (rcvHistFrom)   q = q.gte('received_at', rcvHistFrom)
    if (rcvHistStatus) q = q.eq('status', rcvHistStatus)
    const { data } = await q
    setRcvHistory(data||[]); setRcvHistLoad(false)
  ...
  const rcvHistFiltered = useMemo(() => {
    if (!rcvHistSearch.trim()) return rcvHistory
    const q = rcvHistSearch.toLowerCase()
    return rcvHistory.filter(r => (r.item_name||'').toLowerCase().includes(q) || (r.part_number||'').toLowerCase().includes(q))
  }, [rcvHistory, rcvHistSearch])
  ...
  const loadRequisitions = async () => {
    ...
    if (lastRow) setLastSync(lastRow.imported_at)
    setReqLoad(false)
  }
  ...
  const syncGoogleSheet = async () => {
    ...
    try {
      ...
      let resp
      try { resp = await fetch(csvUrl) } catch { toast.error('Network error fetching sheet'); setSyncing(false); return }
      const contentType = resp.headers.get('content-type') || ''
      if (!resp.ok || contentType.includes('text/html')) {
        toast.error('Sheet is not public. Go to Google Sheets → Share → "Anyone with the link → Viewer"')
      ...
      const reqNoCol  = col('requisition_number','req_number','req_no','requisition_no')
      const partCol   = col('part_number','part_no','part_','part___','code','sku')
      const itemCol   = col('item_name','item','description','product')
      const unitCol   = col('unit','uom')
      const reqQtyCol = col('requested_qty','requested_quantity','requested','qty','quantity')
      const appQtyCol = col('approved_qty','approved_quantity','approved')
      const statusCol = col('status')

      let newCount=0, matchedCount=0, unmatchedCount=0
      const toInsert = []
      ...
      for (const row of sheetRows) {
        ...
        const reqNo  = row[reqNoCol]  || ''
        const reqQty = row[reqQtyCol] || ''
        const appQty = row[appQtyCol] || ''
        const hash   = btoa(unescape(encodeURIComponent(`${reqNo}::${partNumber}::${reqQty}::${appQty}`))).replace(/=/g,'')

        const { data: existing } = await supabase.from('requisitions_import').select('id').eq('sheet_row_hash',hash).single()
        if (existing) { matchedCount++; continue }

        let orderItemId = null
        if (partNumber) {
          const { data: matchedItems } = await supabase.from('order_history_items')
            .select('id').eq('part_number', partNumber)
            .order('created_at',{ascending:false}).limit(1)
          orderItemId = matchedItems?.[0]?.id || null
        }
        if (orderItemId) matchedCount++; else unmatchedCount++

        toInsert.push({
          sheet_row_hash: hash, requisition_number: reqNo||null,
          part_number: partNumber||null, item_name: itemName||null,
          unit: row[unitCol]||null,
          requested_qty: Number(reqQty)||0, approved_qty: Number(appQty)||0,
          status: row[statusCol]||'pending', order_item_id: orderItemId, raw_data: row,
        })
        newCount++
      }
      ...
      if (toInsert.length) {
        ...
        for (const r of toInsert) {
          if (r.order_item_id && r.approved_qty > 0) {
            await supabase.from('order_history_items')
              .update({ approved_qty: r.approved_qty, item_status: 'approved' })
      ...
      setSyncResult({ new:newCount, matched:matchedCount, unmatched:unmatchedCount })
      setLastSync(ts)
      toast.success(`Sync done — ${newCount} new, ${matchedCount} matched, ${unmatchedCount} unmatched`)
      loadRequisitions()
  ...
  const TABS = [
    ...
    { key:'rcv-history', label:'Receiving History' },
  ...
  return (
    <div className="space-y-4">
        ...
        {TABS.map(({key,label}) => (
          <button key={key} onClick={() => switchTab(key)}
            className={`px-4 py-2 text-sm font-medium whitespace-nowrap transition-colors border-b-2 -mb-px
              ${tab===key ? 'border-[#00AEEF] text-[#00AEEF]' : 'border-transparent text-slate-400 hover:text-slate-200'}`}>
            {label}
          </button>
        ))}
      </div>
        ...
        <>
                ...
                {STORES.map(s => (
                  <button key={s} onClick={() => selectStore(s)}
                    className={`px-3 py-1.5 rounded-full text-xs font-semibold border transition-all
                      ${selectedStore===s
                        ? 'bg-teal-600 border-teal-600 text-white shadow-md shadow-teal-900/30'
          ...
          {!loading && rows.length===0 && (
            <div className="card text-center py-14 text-slate-500">
              ...
              <p className="font-medium">No items found for this store</p>
            </div>
          )}
            ...
            <>
              <div className="flex items-center gap-3 flex-wrap">
                <div className="flex gap-2">
                  {[
                    {v:rows.length,     l:'Items',    c:'text-[#00AEEF]'},
                    {v:orderRows.length,l:'To Order',  c:'text-teal-400'},
                    {v:orderRows.reduce((s,r)=>s+Number(r.ordered),0),l:'Units',c:'text-teal-300'},
                  ].map(({v,l,c}) => (
                    <div key={l} className="card-sm py-1.5 px-3 text-center">
                  ...
                  <table className="w-full text-xs">
                    <thead className="border-b border-slate-700 sticky top-0 bg-slate-800 z-10">
                      <tr>
                        ...
                        <th className="px-2 py-2 text-center font-semibold text-slate-400 w-24">Avg/Wk</th>
                      ...
                      {visibleRows.map(row => (
                        <tr key={row.id} className={`transition-colors ${row.ordered>0 ? 'bg-teal-900/10 border-l-2 border-l-teal-600' : 'hover:bg-slate-700/20'}`}>
                          ...
                          <td className="px-2 py-1.5">
                            <p className={`font-medium truncate max-w-[200px] ${row.ordered>0?'text-teal-200':'text-slate-100'}`}>{row.name}</p>
                          </td>
                          <td className="px-2 py-1.5 text-center">
                            <span className="font-bold px-1.5 py-0.5 rounded bg-slate-700 text-teal-300 text-xs">{row.unit}</span>
                          </td>
                          <td className="px-2 py-1.5 text-center">
                            {row.avgConsumption != null
                              ? <span className="text-blue-300 font-medium">{row.avgConsumption}</span>
                              : <span className="text-slate-600 italic text-xs">—</span>}
                          </td>
                          <td className="px-2 py-1.5 text-center">
                            <input type="number" min="0" value={row.physStock===''?'':row.physStock}
                              onChange={e => setPhysStock(row.id, e.target.value)}
                              placeholder="—"
                          ...
                          <td className="px-2 py-1.5 text-center">
                            <div className="flex items-center justify-center gap-1">
                              <button onClick={() => adjustQty(row.id,-1)} className="w-5 h-5 flex items-center justify-center bg-slate-700 hover:bg-slate-600 rounded text-slate-300 shrink-0">
                                <Minus className="w-2.5 h-2.5"/>
                              </button>
                              <input type="number" min="0" value={row.ordered===0?'':row.ordered}
                                onChange={e => setQty(row.id, e.target.value)}
                                placeholder="0"
                                className={`w-14 border rounded px-1 py-1 text-center text-xs font-medium focus:outline-none focus:ring-1 focus:ring-teal-500
                                  ${row.ordered>0 ? 'bg-teal-900/25 border-teal-600 text-teal-200' : 'bg-slate-700 border-slate-600 text-slate-100'}`}/>
                              <button onClick={() => adjustQty(row.id,1)} className="w-5 h-5 flex items-center justify-center bg-slate-700 hover:bg-slate-600 rounded text-slate-300 shrink-0">
                          ...
                          <td className="px-2 py-1.5 text-center">
                            {row.approvedQty > 0
                              ? <span className="text-emerald-400 font-semibold">{row.approvedQty}</span>
                              : <span className="text-slate-600">—</span>}
                          </td>
                          <td className="px-2 py-1.5 text-center">
                            <Badge variant="gray" className="text-xs">draft</Badge>
                          </td>
                          <td className="px-2 py-1.5 text-center">
              ...
              {orderRows.length > 0 && (
                <div className="flex items-center justify-between gap-3 px-4 py-3 card border-teal-700/30 bg-teal-900/10">
                  <p className="text-xs text-teal-300 font-medium">
                    <strong>{orderRows.length}</strong> items ·{' '}
                    <strong>{orderRows.reduce((s,r)=>s+Number(r.ordered),0)}</strong> units
                    {rows.filter(r=>r.physStock!==''&&r.physStock!=null).length > 0 &&
                      <> · <strong>{rows.filter(r=>r.physStock!==''&&r.physStock!=null).length}</strong> physical count entries</>}
                  </p>
                  <div className="flex gap-2">
        ...
        <div className="space-y-3">
          <div className="card-sm flex flex-wrap gap-2 items-center">
            <div className="relative flex-1 min-w-40">
              <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400"/>
              <input className="input pl-8 text-xs w-full" placeholder="Search store, notes…" value={histSearch} onChange={e=>setHistSearch(e.target.value)}/>
            </div>
            <select value={histStatus} onChange={e=>setHistStatus(e.target.value)} className="input text-xs py-1 w-auto">
            ...
            <button onClick={()=>{setHistFrom('');setHistTo('');setHistStatus('');setHistSearch('')}} className="btn-ghost btn-sm">
              <X className="w-3.5 h-3.5"/> Clear
            </button>
          </div>

          {histLoad
            ? <div className="flex justify-center py-14"><div className="w-9 h-9 border-4 border-[#00AEEF] border-t-transparent rounded-full animate-spin"/></div>
            : histDateGroups.length === 0
              ? <div className="card text-center py-14 text-slate-500"><ShoppingCart className="w-10 h-10 mx-auto mb-3 opacity-20"/><p className="font-medium">No orders found</p></div>
              : histDateGroups.map(([date, dateOrders]) => (
                  <div key={date} className="card border border-slate-700/40 p-0">
                    <div className="flex items-center gap-2 px-3 py-2 border-b border-slate-700/40 bg-slate-800/60">
                      ...
                      <p className="font-semibold text-slate-100 text-sm">{fmtDate(date)}</p>
                          ...
                          <div key={order.id}>
                            <div className="flex items-center justify-between flex-wrap gap-2 px-3 py-2">
                              ...
                              <div className="flex gap-1.5 items-center flex-wrap">
                                <button onClick={() => openAddToSavedOrder(order.id)} className="flex items-center gap-1 px-2 py-1 text-xs text-blue-400 border border-blue-700/30 bg-blue-900/10 hover:bg-blue-900/30 rounded transition-colors">
                                  <PlusCircle className="w-3 h-3"/> Add
                                </button>
                                {isExp && oItems.length > 0 && <>
                                  <button onClick={() => exportHistoryExcel(order)} disabled={exportingHistXlsx===order.id} className="flex items-center gap-1 px-2 py-1 text-xs text-emerald-400 border border-emerald-700/30 bg-emerald-900/10 hover:bg-emerald-900/30 rounded transition-colors disabled:opacity-50">
                                    <FileSpreadsheet className="w-3 h-3"/>{exportingHistXlsx===order.id?'…':'Excel'}
                                  </button>
                                  <button onClick={() => exportHistoryPDF(order)} disabled={exportingHistPdf===order.id} className="flex items-center gap-1 px-2 py-1 text-xs text-rose-400 border border-rose-700/30 bg-rose-900/10 hover:bg-rose-900/30 rounded transition-colors disabled:opacity-50">
                                    <FileText className="w-3 h-3"/>{exportingHistPdf===order.id?'…':'PDF'}
                                  </button>
                                  <button onClick={() => emailOrder(order)} disabled={emailingOrder===order.id} className="flex items-center gap-1 px-2 py-1 text-xs text-purple-400 border border-purple-700/30 bg-purple-900/10 hover:bg-purple-900/30 rounded transition-colors disabled:opacity-50">
                                    <Mail className="w-3 h-3"/>{emailingOrder===order.id?'…':'Email'}
                                  </button>
                                </>}
                              </div>
                            </div>
                              ...
                              <div className="border-t border-slate-700/30 overflow-x-auto">
                                <table className="w-full text-xs">
                                  <thead className="bg-slate-800/50"><tr>
                                    <th className="px-2 py-1.5 text-left font-semibold text-slate-400 w-8">SL</th>
                                    <th className="px-2 py-1.5 text-left font-semibold text-slate-400">Part #</th>
                                    <th className="px-2 py-1.5 text-left font-semibold text-slate-400">Item</th>
                                    <th className="px-2 py-1.5 text-center font-semibold text-slate-400">UOM</th>
                                    <th className="px-2 py-1.5 text-center font-semibold text-slate-400">Ordered</th>
                                    <th className="px-2 py-1.5 text-center font-semibold text-slate-400">Approved</th>
                                    <th className="px-2 py-1.5 text-center font-semibold text-slate-400">Received</th>
                                    <th className="px-2 py-1.5 text-center font-semibold text-slate-400">Status</th>
                                  </tr></thead>
                                      ...
                                      return (
                                        <tr key={oi.id} className={sf<=0?'opacity-60':''}>
                                          ...
                                          <td className="px-2 py-1 text-center">
                                            <Badge variant={sf<=0?'green':Number(oi.received_qty)>0?'orange':'yellow'} className="text-xs">
                                              {sf<=0?'Received':Number(oi.received_qty)>0?'Partial':'Pending'}
                                            </Badge>
                                          </td>
                                        </tr>
                                      )
      ...
      {tab==='receiving' && (
        <div className="space-y-3">
          <p className="text-xs text-slate-400">Enter received quantities and flag issues, then save. Changing received qty does NOT automatically update inventory.</p>
          {rcvLoad
            ? <div className="flex justify-center py-14"><div className="w-9 h-9 border-4 border-[#00AEEF] border-t-transparent rounded-full animate-spin"/></div>
            : rcvOrders.length === 0
              ? <div className="card text-center py-14 text-slate-500"><Inbox className="w-10 h-10 mx-auto mb-3 opacity-20"/><p className="font-medium">No pending orders to receive</p></div>
              : rcvOrders.map(order => {
                  const sn    = order.store_name || (order.notes||'').split(' · ')[0] || 'Order'
                  const isExp = rcvExpanded === order.id
                  const oItems= rcvItems[order.id] || []
                  return (
                    <div key={order.id} className="card border border-slate-700/40 p-0">
                      <div className="flex items-center justify-between flex-wrap gap-2 px-3 py-2 border-b border-slate-700/40 bg-slate-800/60">
                        ...
                        {isExp && oItems.length > 0 && (
                          <Button onClick={() => saveReceiving(order)} loading={rcvSaving===order.id}>
                            <CheckCircle2 className="w-3.5 h-3.5"/> Save Receiving
                          </Button>
                        )}
                      </div>
                      {isExp && (
                        <div className="overflow-x-auto">
                          <table className="w-full text-xs">
                            <thead className="bg-slate-800/50"><tr>
                              ...
                              <th className="px-2 py-1.5 text-center font-semibold text-slate-400">Ordered</th>
                              <th className="px-2 py-1.5 text-center font-semibold text-slate-400">Approved</th>
                              <th className="px-2 py-1.5 text-center font-semibold text-slate-400">Received Qty</th>
                              <th className="px-2 py-1.5 text-center font-semibold text-slate-400">Issue</th>
                            </tr></thead>
                            <tbody className="divide-y divide-slate-700/30">
                              {oItems.map(oi => {
                                const inp = rcvInput[oi.id] || { qty:0, issue:'none' }
                                return (
                                  <tr key={oi.id}>
                                    ...
                                    <td className="px-2 py-1.5 text-center">
                                      <input type="number" min="0" value={inp.qty}
                                        onChange={e => {
                                          ...
                                          setRcvInput(p => ({...p, [oi.id]: {...inp, qty:v}}))
                                        }}
                                    ...
                                    <td className="px-2 py-1.5 text-center">
                                      <select value={inp.issue}
                                        onChange={e => setRcvInput(p => ({...p, [oi.id]: {...inp, issue:e.target.value}}))}
      ...
      {tab==='rcv-history' && (
        <div className="space-y-3">
          <div className="card-sm flex flex-wrap gap-2 items-center">
            <div className="relative flex-1 min-w-40">
              <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400"/>
              <input className="input pl-8 text-xs w-full" placeholder="Search item, part #…" value={rcvHistSearch} onChange={e=>setRcvHistSearch(e.target.value)}/>
            </div>
            <select value={rcvHistStatus} onChange={e=>setRcvHistStatus(e.target.value)} className="input text-xs py-1 w-auto">
              <option value="">All Statuses</option>
          ...
          {rcvHistLoad
            ? <div className="flex justify-center py-14"><div className="w-9 h-9 border-4 border-[#00AEEF] border-t-transparent rounded-full animate-spin"/></div>
            : rcvHistFiltered.length === 0
              ? <div className="card text-center py-14 text-slate-500"><Inbox className="w-10 h-10 mx-auto mb-3 opacity-20"/><p className="font-medium">No receiving records yet</p></div>
              : (
                <div className="card overflow-hidden p-0">
                  <div className="overflow-x-auto">
                    <table className="w-full text-xs">
                      <thead className="bg-slate-800 border-b border-slate-700 sticky top-0">
                        <tr>
                          {['Received','Part #','Item Name','UOM','Ordered','Approved','Received Qty','Issue','Status','By'].map(h => (
                            <th key={h} className="px-2 py-2 text-left font-semibold text-slate-400 whitespace-nowrap">{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-700/30">
                        {rcvHistFiltered.map(r => (
                          <tr key={r.id} className="hover:bg-slate-700/20">
                            ...
                            <td className="px-2 py-1.5">
                              {r.issue_type && r.issue_type !== 'none'
                                ? <Badge variant="orange" className="text-xs">{ISSUE_LABELS[r.issue_type]||r.issue_type}</Badge>
                                : <span className="text-slate-600">—</span>}
                            </td>
          ...
          <div className="card-sm space-y-3">
            <div className="flex items-start justify-between gap-4 flex-wrap">
              <div>
                <p className="font-semibold text-slate-100 text-sm flex items-center gap-2">
                  <Link className="w-4 h-4 text-[#00AEEF]"/> Google Sheet Requisitions
                </p>
                <p className="text-xs text-slate-400 mt-1">Syncs approved requisitions from your Google Sheet into the orders system.</p>
            ...
            {syncResult && (
              <div className="grid grid-cols-3 gap-2 border-t border-slate-700/50 pt-3">
                {[
                  {l:'New Imported', v:syncResult.new,       c:'text-teal-400'},
                  {l:'Matched',      v:syncResult.matched,   c:'text-blue-400'},
                  {l:'Unmatched',    v:syncResult.unmatched, c:'text-amber-400'},
                ].map(({l,v,c}) => (
            ...
            <div className="border-t border-slate-700/50 pt-3 text-xs text-slate-500 space-y-1">
              <p className="font-semibold text-slate-400">To enable sync, the Google Sheet must be public:</p>
              <ol className="list-decimal list-inside space-y-0.5 ml-1">
                <li>Open the Google Sheet</li>
                <li>Click <strong className="text-slate-300">Share</strong> → <strong className="text-slate-300">Anyone with the link → Viewer</strong></li>
                <li>Click <strong className="text-slate-300">Sync Requisitions</strong> above</li>
              </ol>
              <p className="mt-1">
                Sheet:{' '}
                <a href={`https://docs.google.com/spreadsheets/d/${SHEET_ID}`} target="_blank" rel="noreferrer"
                  className="text-[#00AEEF] underline underline-offset-2">Open in Google Sheets</a>
              </p>
            </div>
          </div>

          {reqLoad
            ? <div className="flex justify-center py-10"><div className="w-8 h-8 border-4 border-[#00AEEF] border-t-transparent rounded-full animate-spin"/></div>
            : requisitions.length === 0
              ? (
                <div className="card text-center py-12 text-slate-500">
                  <AlertTriangle className="w-10 h-10 mx-auto mb-3 opacity-20"/>
                  <p className="font-medium">No requisitions imported yet</p>
                  <p className="text-xs mt-1">Make the Google Sheet public and click Sync.</p>
                </div>
              )
              : (
                <div className="card overflow-hidden p-0">
                  <div className="overflow-x-auto">
                    <table className="w-full text-xs">
                      <thead className="bg-slate-800 border-b border-slate-700 sticky top-0">
                        <tr>
                          {['Req #','Part #','Item Name','Unit','Requested','Approved','Status','Imported'].map(h => (
                            <th key={h} className="px-2 py-2 text-left font-semibold text-slate-400 whitespace-nowrap">{h}</th>
                          ))}
                        </tr>
                      </thead>
        ...
        <Modal isOpen onClose={() => setShowAddToOrder(null)} title="Add Item to Saved Order" size="sm"
          footer={
            <>
              ...
              <Button onClick={confirmAddToSavedOrder} loading={addingToOrder}>Add Item</Button>
            </>
          }>
          <div className="space-y-3">
            <div>
              ...
              {savedItem ? (
                ...
                <div className="relative">
                  <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400"/>
                  <input className="input pl-8 text-sm w-full" placeholder="Search by name or part #…"
